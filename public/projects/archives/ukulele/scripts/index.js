window.onload = function () {
    ...
    function findInterval(fretNum){
        ...
        for (let i=0; i<=fretNum; i++) {
            returnVal += unit * Math.pow(2,(-i)/12);
        }
    ...
    for(let i=0;i<=12;i++){
        ...
        fret.height = 3
        fret.y = findInterval(i)
        stage.addChild(fret);
        if(circles.indexOf(i)!==-1){
            var mark = new PIXI.Sprite(markTexture)
            mark.y = findInterval(i-1)+(findInterval(i) - findInterval(i-1))/2;
    ...
    //Strings
    for(let i=0;i<4;i++){
        var string = new PIXI.Sprite(shapes.rectangle(strings[i],renderer.height,"rgba(255,255,255,0.6)"));
        stage.addChild(string);
        string.x = i*(renderer.width/4)+renderer.width/8
    }
    var stuff = []
    function draw(){
